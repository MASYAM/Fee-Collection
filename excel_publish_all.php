...
if(isset($_POST['year']) && isset($_POST['month']) && isset($_POST['fullmonth']))
{   
    $year    = $_POST['year'];
    $month = $_POST['month'];
    ...
    //code for data show 
          ...
          $r .= '<table style=" text-align:center;background:whitesmoke;" >' ;
           $r .= '<tr style="background:#0099CC;color:#fff;">' ;
                $r .= '<th style="width:200px;padding:2px;">Date</th>' ;
          ...
          {              
                ...
                $r .= ' >' ;
                 $r .= '<td style="font-size:14px;padding:4px;width:200px;color:#000; " ><b>'.$date_value['date'].'</b></td>' ;
                 foreach ($receipt_items as $receipt_value)
                 {
                     $receipt_items_value = $receipt_value['item'];
                     $result = $db->find_by_sql("SUM(paid) AS total_paid","transaction_receipt_item","year='$year' AND date='{$date_value['date']}' AND receipt_item='{$receipt_items_value}'","");    
                 ...
                 foreach ($extra_added_items as $extra_items_value)
                 {
                     ...
                     $result = $db->find_by_sql("SUM(paid) AS total_paid","transaction_receipt_item","year='$year' AND date='{$date_value['date']}' AND receipt_item='{$extra_added_items_value}'",""); 
                     $r .= '<td style="font-size:14px;padding:4px;width:200px;color:#000;border:1px solid #fff; " ><b>'.$result[0]['total_paid'].'</b></td>' ;
                 }
              $r .= '</tr>';
